 ...
 *
 * @param {ProposeNewImplementationTimeModalProps} props - Dialog state and CR.
...
export default function ProposeNewImplementationTimeModal({
  ...
  const resetFromChangeRequest = useCallback(() => {
    if (!changeRequest) return;
    setProposedStart(changeRequestToDatetimeLocal(changeRequest.startDate));
  }, [changeRequest]);

  useEffect(() => {
    if (open && changeRequest) {
      resetFromChangeRequest();
    }
