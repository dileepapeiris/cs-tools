...
/**
 * Searches instance metrics for a deployment within a date range.
 *
 ...
 */
export default function usePostDeploymentInstancesMetricsSearch(
  ...
  return useQuery<InstanceMetricsResponse>({
    queryKey: [ApiQueryKeys.DEPLOYMENT_INSTANCE_METRICS, deploymentId, payload],
    queryFn: async () => {
      const baseUrl = window.config?.CUSTOMER_PORTAL_BACKEND_BASE_URL ?? "";
      const response = await authFetch(
        `${baseUrl}/deployments/${deploymentId}/instances/metrics/search`,
        {
          method: "POST",
