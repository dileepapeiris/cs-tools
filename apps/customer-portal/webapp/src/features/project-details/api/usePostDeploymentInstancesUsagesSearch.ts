...
/**
 * Searches instance usages for a deployment within a date range.
 *
 ...
 */
export default function usePostDeploymentInstancesUsagesSearch(
  ...
  return useQuery<InstanceUsageResponse>({
    queryKey: [ApiQueryKeys.DEPLOYMENT_INSTANCE_USAGES, deploymentId, payload],
    queryFn: async () => {
      const baseUrl = window.config?.CUSTOMER_PORTAL_BACKEND_BASE_URL ?? "";
      const response = await authFetch(
        `${baseUrl}/deployments/${deploymentId}/instances/usages/search`,
        {
          method: "POST",
