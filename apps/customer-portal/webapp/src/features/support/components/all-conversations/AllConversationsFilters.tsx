 ...
 *
 * @param {AllConversationsFiltersProps} props - Filter values, metadata, and change handler.
 * @returns {JSX.Element} The rendered filter dropdowns.
 */
export default function AllConversationsFilters({
  filters,
  filterMetadata,
  onFilterChange,
}: AllConversationsFiltersProps): JSX.Element {
  ...
  return (
    <Grid container spacing={2} sx={{ mt: 1 }}>
      {ALL_CONVERSATIONS_FILTER_DEFINITIONS.map((def) => {
