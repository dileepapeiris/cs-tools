 ...
 *
 * @param {AllConversationsSearchBarProps} props - Search term, change handler, and filters.
 * @returns {JSX.Element} The rendered search bar.
 */
export default function AllConversationsSearchBar({
  searchTerm,
  onSearchChange,
  isFiltersOpen,
  onFiltersToggle,
  ...
  onFilterChange,
  onClearFilters,
}: AllConversationsSearchBarProps): JSX.Element {
      ...
      <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
        <Box sx={{ position: "relative", flex: 1 }}>
          <TextField
            fullWidth
            size="small"
            placeholder="Search chats by message, ID, or category..."
            value={searchTerm}
