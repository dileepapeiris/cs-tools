...
# + return - Deployment search response or error
public isolated function searchDeployments(string token, DeploymentSearchPayload payload)
...
public isolated function searchDeployedProducts(string token, DeployedProductSearchPayload payload)
    ...
    return csEntityClient->/deployed\-products/search.post(payload, generateHeaders(token));
}
